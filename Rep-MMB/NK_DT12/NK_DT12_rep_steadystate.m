function [ys,params,check] = NK_DT12_rep_steadystate(ys,exo,M_,options_);  %% added additional input, to prevent error for missing exogenous vars
%check=0;
...
for iter = 1:length(M_.params) %update parameters set in the file
  eval([ 'params(' num2str(iter) ') = ' M_.param_names{iter} ';' ])
    ...
    for kkk=1:(size(M_.endo_names,1)-size(M_.aux_vars,2))
        %ys(kkk)=eval(strcat(deblank(M_.endo_names(kkk,:)),'_ss'));
        ys(kkk)=eval(strcat(M_.endo_names{kkk},'_ss'));
    end
