function [ys,params,check] = US_CMR14_rep_steadystate(ys,exo,M_,options_)
...
%if cee == 0
gamma    = gamma_p;
xi8      = 1; log_xi8=log(xi8);
xi7      = 1; log_xi7=log(xi7);
xi6      = 1; log_xi6=log(xi6);
xi5      = 1; log_xi5=log(xi5);
xi4      = 1; log_xi4=log(xi4);
xi3      = 1; log_xi3=log(xi3);
xi2      = 1; log_xi2=log(xi2);
xi1      = 1; log_xi1=log(xi1);
xi0      = 1; log_xi0=log(xi0);
e_g      = 0;
%end
u        = 1;
uf       = u;
...
[rk,n,kbar,h,i,wtilde,d,c,lambdaz,yz,sigma,omegabar,phi,s,pitild,pstar,piw, ...
    ...
    log_zetac=log(zetac);
    log_lambdaz=log(lambdaz);
    log_lambdazf=log(lambdaz);
    log_term=log(term);
    log_pi=log(pi);
    log_muzstar=log(muzstar);
catch
    
    ys=[];
    error('(cmr_steadystate) failed to compute a steady state')
    check=1;
    return;
    
end
if wtilde <0 | i <0 | kbar <0 | c<0 | uzc<0 | lambdaz<0
    error(' (steadystate) w or i or k or y or c or uzc<0')
end
